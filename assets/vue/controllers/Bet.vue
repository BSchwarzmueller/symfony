...
const betPoints = ref<number | null>(props.betPoints)
...
const processBet = async () => {
  loading.value = true
  loadingMessage.value = 'Processing bet...'

  try {
    const response = await axios.get('/bet/process/' + props.betId)

    if (response.status === 200) {
      betPoints.value = response.data.points
    }
  } catch (error) {
    loadingMessage.value = 'Error processing bet'
...
<template>
  <div class="bet-container">
    ...
    <div v-if="props.awayGoals && props.homeGoals" class="bet-game">
      <div>Ergebnis:</div>
      <div class="result">
        <span :class="{
          'hit' : props.homeGoals === homeGoals,
          'miss' : props.homeGoals !== homeGoals,
        }"
            class="number">{{ props.homeGoals }}</span>
        <span class="separator">:</span>
        <span  :class="{
          'hit' : props.awayGoals === awayGoals,
          'miss' : props.awayGoals !== awayGoals,
        }"
               class="number">{{ props.awayGoals }}</span>
      </div>
      <div v-if="betPoints === null && props.betStatus === 'open'" class="claim">
        <button class="button" @click="processBet()">Claim</button>
      </div>
    ...
    <div v-if="!interaction" class="bet-community">
